 ...
 *
 * Because the cells are painted rather than transparent, the colours come from
 * the entry tokens, so a row dropped on the navy ground stays on navy instead
 * of punching cream tiles through it.
 */
  ...
  return (
    <div
      className={`-ml-4 grid gap-px ${columnsFor(stats.length)} ${
        stats.length > 1 ? "bg-[var(--entry-rule)]" : ""
      }`}
    >
      {stats.map((stat) => (
        <div key={stat.label} className="bg-[var(--entry-tile)] px-4 py-6">
          <p
            className={`font-display text-[var(--entry-accent)] ${
              compact
          ...
          <p className="mt-2 text-[0.72rem] font-medium uppercase leading-snug tracking-[0.14em] text-[var(--entry-stat-label)]">
            {stat.label}
          </p>
        </div>
      ))}
